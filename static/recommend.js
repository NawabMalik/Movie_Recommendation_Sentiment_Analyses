$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '51490b7ba92abe5340fcfb2b9d86e703';
    var title = $('.movie').val();
    if (title=="") {
...
function movie_recs(movie_title,movie_id,my_api_key){
  $.ajax({
    ...
    success: function(recs){
      if(recs=="Sorry! The Movie you requested in not present in our database. Please check the spelling or try other Movies."){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
